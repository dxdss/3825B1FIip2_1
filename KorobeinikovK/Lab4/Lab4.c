...
void init_prod() {
	struct Product init_prod[] = {
		{"1564", "Bread", 40, 5}, {"9318", "Tea", 120, 25},
		{"1488", "Pasta", 65, 40}, {"2008", "Cookie", 200, 15},
		{"0914", "Chocolate", 175, 12}
	};
	for (int i = 0; i < 5; i++) {
		prods[i] = init_prod[i];
	}
...
int find_prod(const char* barcode) {
	for (int i = 0; i < prod_count; i++) {
		...
		for (int j = 0; j < BARCODE_SIZE + 1; j++) { // +1 чтобы сравнить и нулевой символ
